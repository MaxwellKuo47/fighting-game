...
import { drawGlassAstrologerMaterialTexture, drawGlassAstrologerTexture } from './texture.ts';
...
const data = {
  ...
  talent: { id: 'refraction_astrolabe', name: '入射角', desc: '普攻射出三連星片；星片穿過星鏡時原球會繼續穿透，鏡面同時射出一顆較大的反射球。', max: 3, maxMirrors: 4 },
  ...
  skill1: { name: '立鏡', type: 'glass_mirror', range: 180, length: 210, thickness: 18, charges: 6, maxMirrors: 4, lifetime: 9, sideAngle: 0.3, manaCost: 8, cd: 2.4, color: '#d8f7ff', vfx: 'glass_astrologer_mirror' },
  skill2: { name: '折星光束', type: 'glass_beam', duration: 0.75, tick: 0.25, directionCount: 8, pulseDmg: 5, range: 760, width: 30, widthPerReflect: 8, maxReflects: 2, reflectBonus: 0.35, markBonusPerStack: 4, mirrorPushRadius: 520, mirrorPushDistance: 140, manaCost: 24, cd: 8.5, color: '#9ee8ff', vfx: 'glass_astrologer_ray' },
...
export class GlassAstrologerCharacter extends BaseCharacter {
  constructor() {
    super(data, {
      ...
      paintMaterialTexture: drawGlassAstrologerMaterialTexture,
      loadVfx: () => undefined,
      tick: tickGlassBeam,
    });
  }
